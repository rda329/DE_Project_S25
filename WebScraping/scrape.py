...
def scrape_google_bing(query,engine, page_counter):
    ...
    pdf_path = rf"WebScraping\ScreenCaptures\{engine}_page_{page_counter}.pdf"
    output_path = rf"WebScraping\URL_JSON\{engine}_results.json"
    extract_search_results(pdf_path, output_path)

def check_number_urls(engine):
    with open(rf"WebScraping\URL_JSON\{engine}_results.json", 'r') as file:
        data = json.load(file)
...
def scrape_web(query, num_results): #scrape AT LEAST num_results 
    ...
    output_file = r"WebScraping\URL_JSON\duckduckgo_results.json"
    ...
    output_file = r"WebScraping\URL_JSON\yahoo_results.json"
    save_to_json(data, output_file)


query = "cool smart phones"
num_results = 20
